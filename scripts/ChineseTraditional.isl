...
InvalidParameter=某個無效的參數已被傳遞到了命令列：%n%n%1
SetupAlreadyRunning=安裝程式已經在執行。
WindowsVersionNotSupported=本安裝程式並不支援目前在電腦所執行的 Windows 版本。
...
InvalidPath=您必須輸入完整的路徑名稱及磁碟機代號。%n%n例如 C:\App 或 UNC 路徑格式 \\伺服器\共用資料夾。
...
ChangeDiskTitle=安裝程式需要下一張光碟
SelectDiskLabel2=請插入光碟 %1，然後按「確定」。%n%n如果檔案不在以下所顯示的資料夾之中，請輸入正確的資料夾名稱或按 [瀏覽] 選取。
PathLabel=路徑(&P)：
FileNotInDir2=檔案「%1」無法在「%2」找到。請插入正確的光碟或選擇其它的資料夾。
SelectDirectoryLabel=請指定下一張光碟的位置。
