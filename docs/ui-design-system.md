...
Pin **forui 0.25.x** (not 0.26+). Forui 0.26 pulls in `material_ui`, which deadlocks `riverpod_generator` / `build_runner` in this repo. Generated `*.g.dart` / `*.freezed.dart` stay gitignored and are produced in CI.
