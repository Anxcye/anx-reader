...
/// Preserve raw Gemini [Content] (including thought signatures on function
/// calls) across tool-call round-trips. Keyed by tool-call fingerprint.
final Map<String, g.Content> _geminiToolContentCache = {};

String _geminiToolContentCacheKey(AIChatMessage msg) {
  if (msg.toolCalls.isEmpty) return '';
  return msg.toolCalls
      .map((t) => '${t.id}|${t.name}|${t.argumentsRaw}')
      .join(';');
}

void _cacheGeminiToolContent(AIChatMessage msg, g.Content? content) {
  if (content == null || msg.toolCalls.isEmpty) return;
  final key = _geminiToolContentCacheKey(msg);
  if (key.isNotEmpty) {
    _geminiToolContentCache[key] = content;
  }
}

g.Content? _cachedGeminiToolContent(AIChatMessage msg) {
  final key = _geminiToolContentCacheKey(msg);
  if (key.isEmpty) return null;
  return _geminiToolContentCache[key];
}
...
extension ChatMessagesMapper on List<ChatMessage> {
  ...
  g.Content _mapAIChatMessage(final AIChatMessage msg) {
    final cached = _cachedGeminiToolContent(msg);
    if (cached != null) {
      return cached;
    }
